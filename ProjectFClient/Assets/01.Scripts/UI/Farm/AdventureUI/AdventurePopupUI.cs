...
namespace ProjectF.UI.Adventure
{
    public class AdventurePopupUI : PoolableBehaviourUI
    {
        ...
        [SerializeField] private AddressableAsset<ExploreFarmerInfoElementUI> _farmerElementPrefab;
        ...
        public void Initialize(AdventureData data)
        {
            ...
            RegisterLootItemIcon(data.adventureAreaID);
            SpawnExploreFarmerElement();
        }

        private void RegisterLootItemIcon(int areaID)
        {
            var lootTable = DataTableManager.GetTable<AdventureLootTable>();
            var lootRow = lootTable.GetRow(areaID);
        ...
        {
            _farmerElementPrefab.Initialize();
            ...
            {
                var element = await PoolManager.SpawnAsync<ExploreFarmerInfoElementUI>(_farmerElementPrefab.Key);
